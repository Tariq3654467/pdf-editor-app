    ...
    try {
        ...
        for (fz_stext_block *block = stext->first_block; block && !found; block = block->next) {
            ...
            for (fz_stext_line *line = block->u.t.first_line; line && !found; line = line->next) {
                ...
                // Check if this line matches the target position
                if (std::abs(lineCenterX - targetX) < 10.0f && 
                    std::abs(lineCenterY - targetY) < 10.0f) {
                    textBbox = line_bbox;
                    
                    // Get font info from first char
                    if (line->first_char && line->first_char->font) {
                        fontSize = line->first_char->size;
                        const char *name = fz_font_name(ctx, line->first_char->font);
        ...
        if (!found) {
            LOGE("Text object not found at position");
